...
describe('SourceAttributionAgent', () => {
  ...
  const satMock = {
    getNearestReading: jest
      .fn()
      .mockResolvedValue({ no2: 0.0002, thermalAnomaly: 350 }),
  };
  const luMock = {
    getLandUseForZone: jest.fn().mockResolvedValue({
      industrialPct: 40,
      constructionPct: 10,
      roadDensity: 5,
    }),
  ...
  it('returns percentages summing to 100 with confidence in [0,1]', async () => {
    const zones = [
      { id: 'z1', zoneName: 'Ward A', city: 'Delhi', roads: [{ id: 'r1' }] },
    ];
    const out = await agent.attributeSources(
      zones as any,
      { r1: 80 },
      { Delhi: 1.5 },
    );
    ...
    const sum =
      a.traffic + a.industry + a.construction + a.biomassBurning + a.background;
    expect(sum).toBe(100);
