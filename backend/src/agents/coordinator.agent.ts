...
export class CoordinatorAgent {
  ...
  async handleScheduledSweep() {
    this.logger.log(
      'Coordinator Agent [Schedule]: Triggering scheduled multi-agent orchestration sweep...',
    );
    await this.coordinateSweep();
    this.logger.log(
      'Coordinator Agent [Schedule]: Scheduled multi-agent sweep completed.',
    );
  }
  ...
  private async withTimeout<T>(
    ...
    return Promise.race([promise, this.timeout(ms)]).catch((err) => {
      this.logger.warn(
        `Coordinator Agent: ${label} failed/timed out: ${err.message}`,
      );
      throw err;
    });
  }
  ...
  async coordinateSweep() {
    this.logger.log(
      'Coordinator Agent: Starting orchestration sweep across all 9 agents...',
    );

    try {
      const rawAqiReadings = await this.prisma.aqiReading.findMany({
        take: 50,
        orderBy: { timestamp: 'desc' },
      });
      const rawWeatherReadings = await this.prisma.weatherData.findMany({
        take: 50,
        orderBy: { timestamp: 'desc' },
      });
      const rawTrafficReadings = await this.prisma.trafficData.findMany({
        take: 50,
        orderBy: { timestamp: 'desc' },
      });
      ...
      const [validatedAqi, validatedWeather, validatedTraffic] =
      ...
      this.logger.log(
        `Coordinator Agent: Group 1 completed in ${Date.now() - startG1}ms`,
      );
      ...
      this.logger.log(
        `Coordinator Agent: Group 2 completed in ${Date.now() - startG2}ms`,
      );
      ...
      const [attributions, interventions, advisories] = await this.withTimeout(
        Promise.all([
          this.sourceAttributionAgent.attributeSources(
            zones,
            trafficMap,
            windSpeedMap,
          ),
      ...
      this.logger.log(
        `Coordinator Agent: Group 3 completed in ${Date.now() - startG3}ms`,
      );

      const totalDuration = Date.now() - startG1;
      this.prometheusService.observeAgentSweep(totalDuration);
      this.prometheusService.observeAgentStep('group1', Date.now() - startG1);
      this.prometheusService.observeAgentStep('group2', Date.now() - startG2);
      this.prometheusService.observeAgentStep('group3', Date.now() - startG3);
      this.logger.log(
        `Coordinator Agent: Multi-agent orchestration sweep completed in ${totalDuration}ms.`,
      );
      ...
      this.logger.error(
        `Coordinator Agent: Orchestration sweep failed: ${err.message}`,
      );
      throw err;
    }
  }
