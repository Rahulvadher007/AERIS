...
export class CitizenAdvisoryAgent {
  ...
  async generateAdvisories(
    zones: any[],
    avgCityAqi: Record<string, number>,
    avgCityForecast: Record<string, number>,
  ) {
    const out = [];
    // Limit to first 20 zones to avoid LLM timeout
    const zonesToProcess = zones.slice(0, 20);
    for (const zone of zonesToProcess) {
      ...
      const vuln = await this.vulnerability
        .computeScore(zone)
        .catch(() => ({ score: 0.5 }));
      ...
      const record = {
        ...
        message,
        ttsScript: message,
        recommendedActions: ['Limit outdoor exertion', 'Use masks if outside'],
        forecastAQI: fAQI,
