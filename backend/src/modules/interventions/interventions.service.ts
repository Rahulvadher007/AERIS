...
export class InterventionsService {
  ...
  async generateInterventions() {
    ...
    const latestForecasts = await this.prisma.$queryRaw<Array<{ stationId: string; forecastAQI: number; id: string }>>`
    ...
    for (const zone of zones) {
      ...
      const trafficCongestion = validRoads > 0 ? avgTraffic / validRoads : 35; // default if no roads
      const hotspotCount = zone.hotspots.length;

      // Extract meteorological and pollutant factors
      const currentAQI = hotspotCount > 0
        ? Math.round(zone.hotspots.reduce((sum: number, h: any) => sum + h.aqi, 0) / hotspotCount)
        : (avgCityAqi[zone.city] || 120);
  ...
  private async enrichInterventions(interventions: any[]) {
    ...
    const latestAqiReadings = await this.prisma.$queryRaw<Array<{ stationId: string; aqi: number }>>`
    ...
    const latestWeather = await this.prisma.$queryRaw<Array<{ stationId: string; windSpeed: number }>>`
    ...
    for (const [city, aqis] of Object.entries(cityStationsList)) {
      cityAqiMap[city] = Math.round(aqis.reduce((a, b) => a + b, 0) / aqis.length);
    }
    ...
    for (const [city, aqis] of Object.entries(cityForecastsList)) {
      cityForecastMap[city] = Math.round(aqis.reduce((a, b) => a + b, 0) / aqis.length);
    }
    ...
    return unique.map(intervention => {
      ...
      if (zone.hotspots && zone.hotspots.length > 0) {
        currentAQI = Math.round(zone.hotspots.reduce((sum: number, h: any) => sum + h.aqi, 0) / zone.hotspots.length);
      } else {
        currentAQI = cityAqiMap[zone.city] || 120;
      }

      // Calculate forecast AQI
      const forecastAQI = cityForecastMap[zone.city] || Math.round(currentAQI * 1.15);
  ...
  async getDashboard(city?: string) { return this.repository.getDashboardMetrics(city); }
}
