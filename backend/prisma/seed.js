...
function generateUniqueCode(city) {
...
function generateZonePolygon(lat, lon, quadrant) {
    const dLat = 0.04;
    const dLon = 0.04;
...
function generateRoadLineString(lat, lon, quadrant) {
    const dLat = 0.04;
    const dLon = 0.04;
    let minLat = lat, maxLat = lat + dLat;
    let minLon = lon, maxLon = lon + dLon;
...
async function main() {
    ...
    await prisma_1.prisma.$transaction([
        prisma_1.prisma.trafficData.deleteMany(),
        prisma_1.prisma.road.deleteMany(),
        prisma_1.prisma.weatherData.deleteMany(),
        prisma_1.prisma.aqiReading.deleteMany(),
        prisma_1.prisma.forecastResult.deleteMany(),
        prisma_1.prisma.station.deleteMany(),
        prisma_1.prisma.hotspot.deleteMany(),
        prisma_1.prisma.intervention.deleteMany(),
        prisma_1.prisma.zone.deleteMany()
    ]);
    console.log('Database tables cleared successfully.');
    const zonesToInsert = [];
    const stationsToInsert = [];
    const roadsToInsert = [];
    const stationToRoadMap = {};
    const cityInfoMap = {};
    let zoneCounter = 1;
    let roadIndex = 1;
    ...
    for (const cityData of CITIES_DATA) {
        ...
        for (let zoneIndex = 0; zoneIndex < 4; zoneIndex++) {
            const zoneId = (0, crypto_1.randomUUID)();
        ...
        for (let sIndex = 0; sIndex < stationCount; sIndex++) {
            ...
            const stationId = (0, crypto_1.randomUUID)();
            ...
            const roadId = (0, crypto_1.randomUUID)();
    ...
    await prisma_1.prisma.zone.createMany({ data: zonesToInsert });
    console.log('Inserting Monitoring Station records in bulk...');
    await prisma_1.prisma.station.createMany({ data: stationsToInsert });
    console.log('Inserting Road segment records in bulk...');
    await prisma_1.prisma.road.createMany({ data: roadsToInsert });
    ...
    async function worker() {
        while (true) {
            ...
            await prisma_1.prisma.aqiReading.createMany({ data: aqiBatch });
            await prisma_1.prisma.weatherData.createMany({ data: weatherBatch });
            await prisma_1.prisma.trafficData.createMany({ data: trafficBatch });
            await prisma_1.prisma.forecastResult.createMany({ data: forecastBatch });
    ...
    await prisma_1.prisma.hotspot.createMany({ data: hotspotData });
    await prisma_1.prisma.intervention.createMany({ data: interventionData });
